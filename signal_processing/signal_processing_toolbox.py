...
def process_ecg_signal(data_ecg_res,period_start,period_end,file_name,drop_missing=False):
...
def process_rsp_signal(data_ecg_res,period_start,period_end,file_name,drop_missing=False):
...
def merge_ecg_rsp(df_ecg_features,df_rsp_features, new_name):
    ...
    if not os.path.exists("merged_processed_files"):
        os.mkdir("merged_processed_files")
    new_df.to_csv("merged_processed_files/"+new_name+".csv")
    
    return(new_df)


def run_ecg_res_processing(data_ecg_res,period_start,period_end,file_name,drop_missing=False):
    
    df1=process_ecg_signal(data_ecg_res,period_start,period_end,"ECG_Features"+file_name,drop_missing)
    df2=process_rsp_signal(data_ecg_res,period_start,period_end,"RSP_Features"+file_name,drop_missing)    
        
    combineddf= merge_ecg_rsp(df1,df2,"ECG_RSP_Features"+file_name)
